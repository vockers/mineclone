...
void Shader::build(const char *vertex_code, const char *fragment_code)
{
    ...
    if (!success) {
        ...
        throw std::runtime_error("Error: Shader::build failed: vertex shader compilation failed: " +
                                 std::string(info_log));
    }
    ...
    if (!success) {
        ...
        throw std::runtime_error(
            "Error: Shader::build failed: fragment shader compilation failed: " +
            std::string(info_log));
    }
    ...
    if (!success) {
        ...
        throw std::runtime_error("Error: Shader::build failed: shader program linking failed: " +
                                 std::string(info_log));
    }
